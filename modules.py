"""
Components of SASRec model architecture

This files remains unmodified from the source
...
def positional_encoding(dim, sentence_length, dtype=tf.float32):
    ...
    return tf.convert_to_tensor(
        value=encoded_vec.reshape([sentence_length, dim]), dtype=dtype
    )
...
def multihead_attention(
    ...
    with tf.compat.v1.variable_scope(scope, reuse=reuse):
        ...
        key_masks = tf.tile(
            tf.expand_dims(key_masks, 1), [1, tf.shape(input=queries)[1], 1]
        )  # (h*N, T_q, T_k)
        ...
        if causality:
            ...
            masks = tf.tile(
                tf.expand_dims(tril, 0), [tf.shape(input=outputs)[0], 1, 1]
            )  # (h*N, T_q, T_k)

            paddings = tf.ones_like(masks) * (-(2**32) + 1)
            outputs = tf.compat.v1.where(
                tf.equal(masks, 0), paddings, outputs
            )  # (h*N, T_q, T_k)
        ...
        query_masks = tf.tile(
            tf.expand_dims(query_masks, -1), [1, 1, tf.shape(input=keys)[1]]
        )  # (h*N, T_q, T_k)
        outputs *= query_masks  # broadcasting. (N, T_q, C)

        # Dropouts
        outputs = tf.compat.v1.layers.dropout(
            outputs, rate=dropout_rate, training=tf.convert_to_tensor(value=is_training)
        )
